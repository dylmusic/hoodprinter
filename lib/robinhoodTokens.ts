...
const TRENDING_TOKENS: RhToken[] = [
  ...
  { chainId: siteConfig.chain.chainId, address: "0x385b36Ff682Ab4C76E7c37A66b96aABC466471d5", symbol: "POOLS", name: "pools.trade", decimals: 18, logo: "" }, // V4 (real liquidity ~$232K) is the actual trading venue — NOT the V3 pair add-token.mjs found (~$41K, secondary), routes via Relay same as CATSTR/FRONG. No logo: Relay's own currencies API has no logoURI and flags this token verified:false; DexScreener's CDN logo URL 422s (dead). Deliberately did NOT substitute the real pools.trade site's own favicon as a stand-in — this specific token's on-chain metadata self-declared that URL with zero domain-ownership verification (any creator on this permissionless factory can do the same; a sibling same-named token left the field blank), so using it here would visually imply an endorsement that isn't established. Falls through to the generic fallback badge instead, same graceful-degradation path already documented above for dead logo URLs.
