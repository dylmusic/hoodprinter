...
const TRENDING_TOKENS: RhToken[] = [
  ...
  { chainId: siteConfig.chain.chainId, address: "0x385b36Ff682Ab4C76E7c37A66b96aABC466471d5", symbol: "POOLS", name: "pools.trade", decimals: 18, logo: "https://pools.trade/apple-touch-icon.png" }, // V4 (real liquidity ~$232K) is the actual trading venue — NOT the V3 pair add-token.mjs found (~$41K, secondary), routes via Relay same as CATSTR/FRONG. No logo via Relay's live /currencies/v2 lookup (fetchRelayTokenLogo) — comes back logoURI:"" / verified:false for this address, so the usual live-fetch source has nothing indexed for it; DexScreener's CDN logo URL also 422s (dead). Logo instead pulled from this token's own on-chain creation metadata (the image field self-declared in its TokenCreated event) — verified live (200 OK) directly against the real pools.trade domain.
